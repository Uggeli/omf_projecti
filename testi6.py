...
def save_model(model):
    ...
    torch.save(critic.state_dict(),
               fr'G:\Kdaus\pyyttoni\Omf_projekti2\fighter_models\{timestr}{model}.tar')
...
def optimize():
    ...
    # if len(player2_memory) < BATCH_SIZE:
    #     return

    # p1transitions = player2_memory.sample(BATCH_SIZE)
    # p1batch = Transition(*zip(*p1transitions))
    ...
    # state_action_values = player2(state_batch).gather(1, action_batch)

    # next_state_values = torch.zeros(BATCH_SIZE, device=device)
    # next_state_values[non_final_mask] = player2_target(non_final_next_states).max(1)[0].detach()

    # expected_state_action_values = (next_state_values * GAMMA) + reward_batch

    # loss = F.smooth_l1_loss(state_action_values, expected_state_action_values.unsqueeze(1))

    # player2_optim.zero_grad()
    # loss.backward()
    ...
    if env.match() == 'fight':
        ...
        while in_fight:
            ...
            if p1 >= 200:
                p2_score = 100
                p1 = p1_last
                # save_model(f'p2{sum(p2_total_score)}')
                gameover = True
            else:
                p2_score = p1_last - p1
                p1_last = p1
            ...
            if p2 >= 200:
                p1_score = 100
                p2 = p2_last
                save_model(f'p1{sum(p1_total_score)}')
                gameover = True
            else:
                p1_score = p2_last - p2
                p2_last = p2
            ...
            player1_memory.push(state.to('cpu'), p1_action.to('cpu'), next_state.to('cpu'),
                                torch.tensor([p1_score], device='cpu'))
            # player2_memory.push(state.to('cpu'), p2_action.to('cpu'), next_state.to('cpu'),
