...
player1.load_state_dict(torch.load(model_to_load))
...
p2_last_scores = deque([1], maxlen=50)
...
while training:
    if env.match() == 'fight':
        ...
        p1_action = torch.tensor([[random.randrange(6)]],
                                 device=device,
                                 dtype=torch.long)
        last_action = p1_action
        
        while in_fight:
            if env.match() != 'fight' or gameover:
                keyUp(moves_list_p1[last_action.item()])
                keyUp(moves_list_p1[p1_action.item()])
                in_fight = False
                print('!GAME OVER!')
            ...
            if sum(p1_last_scores) == 0:
                p1_score = -1.
                player1.steps_done = 0

            if p1_score > 100 or p1_score < -100:
                p1_score = 0.

            print(f'score:{p1_score}\
                move:{human_readable_action_list[p1_action]}\
                    total score:{sum(p1_total_score)}')
            ...
            if last_action == p1_action:
                pass
            else:
                keyDown(moves_list_p1[p1_action.item()])
                keyUp(moves_list_p1[last_action.item()])
